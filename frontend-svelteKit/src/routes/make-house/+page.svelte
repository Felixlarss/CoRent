<script lang="ts">
	...
	let house_rent: number = $state(NaN);
	let house_m2: number = $state(NaN);
	...
	async function addEmptyRoom() {
		rooms.push({ room_name: '', room_m2: NaN, room_id: String(temp_room_id) });
		temp_room_id++;
	}
